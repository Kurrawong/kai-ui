...
interface Props {
  ...
  layersOverrideStyleFunction?: Function | undefined;
}
...
const props = withDefaults(defineProps<Props>(), {
  ...
  layersOverrideStyleFunction: undefined,
});
...
const layersRef = ref<Array<InstanceType<typeof Layers.OlVectorLayer>> | null>(
  null,
);
const layerSourcesRef = ref<Array<
  InstanceType<typeof Sources.OlSourceVector>
> | null>(null);
const clickSelectRef = ref<InstanceType<
  typeof Interactions.OlInteractionSelect
> | null>(null);
const drawSourceRef = ref<InstanceType<typeof Sources.OlSourceVector> | null>(
  null,
);
...
const drawstart = (event: any) => {
  emit(
    "drawstart",
    geoJSONFormat.writeFeature(event.feature, {
...
const processLayers = (newLayers: any[]) => {
  ...
  for (const layer of newLayers) {
    ...
    for (const feature of features) {
      ...
      if (feature.geoJSON) {
        geoJSONFeature = geoJSONFormat.readFeature(feature.geoJSON, {
          dataProjection: props.projection,
        });
      } else if (feature.wkt) {
        geoJSONFeature = wktFormat.readFeature(feature.wkt, {
          dataProjection: props.projection,
        });
      }
...
function overrideStyleFunction(feature: any, currentStyle: any) {
  if (
    feature.data?.iri &&
    feature.data?.iri === selectedFeature.value?.data?.iri
  ) {
    return new Style({
...
<template>
  <div class="kai-map">
    ...
    >
        ...
        <Styles.OlStyle
          :overrideStyleFunction="
            (feature, currentStyle) =>
              layersOverrideStyleFunction(feature, currentStyle, layer)
          "
        >
          <Styles.OlStyleStroke
            :color="layer.strokeColor || drawStyle.strokeColor"
            :width="layer.strokeWidth || drawStyle.strokeWidth"
          ></Styles.OlStyleStroke>
          <Styles.OlStyleFill
            :color="layer.fillColor || drawStyle.fillColor"
          ></Styles.OlStyleFill>
          <Styles.OlStyleCircle :radius="layer.radius || drawStyle.radius">
            <Styles.OlStyleFill
              :color="layer.circleColor || drawStyle.circleColor"
            ></Styles.OlStyleFill>
      ...
      <div
        class="custom-map-controls ol-unselectable ol-control ol-bar ol-group flex flex-row"
      ...
      >
        ...
        <button
          v-if="clickThroughModeEnabled"
          type="button"
          class="active"
          name="clickThroughButton"
          title="Disable to only select top feature on click"
        ...
        <button
          ...
          type="button"
          name="clickThroughButton"
          title="Enable to select all overlapping features on click"
          @click="toggleClickThroughMode"
        >
          &DoubleDownArrow;
        </button>
      ...
      <Map.OlOverlay
        v-if="
          clickThroughModeEnabled && selectedFeatures && selectedFeatures.length
        "
      ...
      <Map.OlOverlay
        v-else-if="selectedFeature && selectedFeature.name"
        :position="selectedPosition || center"
        positioning="bottom-center"
        :stopEvent="true"
      >
        <template v-slot="">
